...
func extractIdentityFromCert(cert *x509.Certificate) (string, error) {
	...
	for _, ext := range cert.Extensions {
		if ext.Id.String() == "1.3.6.1.4.1.57264.1.8" {
			...
			if err := validateIdentityString(identity); err != nil {
				return "", err
			}
			return identity, nil
		}
...
// validateIdentityString validates an identity string to prevent injection attacks.
func validateIdentityString(identity string) error {
